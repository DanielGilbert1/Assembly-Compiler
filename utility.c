...
bool isCommentOrEmpty(lineInfo *line)
{
	...
	if (*startOfText == ';')
	{
		/* Illegal comment - ';' isn't the first character in line */
		printError(line->lineNum, "Comments must start with ';'");
		line->isError = TRUE;
		return TRUE;
	}
...
/* Returns if the command is a directive. */
...
{
	/* check if the string parameter is enclosed in quotes */
...
/* Returns True if the num is a legal number param, and save it's value in *value. */
