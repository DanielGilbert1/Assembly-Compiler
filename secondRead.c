...
memoryWord getCmdMemoryWord(lineInfo line)
{
	...
	if (line.isJmp)
	{
		memory.valueBits.cmdBits.src = (eraType)ABSOLUTE; /*Absolute*/
		memory.valueBits.cmdBits.dest = (eraType)RELOCATABLE; /*Relocatable*/
	}
	return memory;
}
...
bool addLineToMemory(int *memoryArr, int *memoryCounter, lineInfo *line)
{
	...
	if (!line->isError && line->cmd != NULL)
	{
		...
		{
            memoryWord memory = { 0 };
			line->jumpLabel = getLabel(line->jumpLabelName);
			if (line->jumpLabel && line->jumpLabel->isExtern)
			{
				memory.era = EXTERNAL;
			}
			else
			{
				memory.era = RELOCATABLE;
			}
			memory.valueBits.value = line->jumpLabel->address;
