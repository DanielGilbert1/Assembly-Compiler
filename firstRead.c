...
void parseOpInfo(operandInfo *operand, int lineNum) {
    ...
    if (isWhiteSpaces(operand->str)) {
        printError(lineNum, "One of the operands is missing.");
        operand->type = INVALID;
        return;
    }

    /* Check if the type is NUMBER */
    if (*operand->str == '-' || (*operand->str >= '0' && *operand->str <= '9')) {
...
void parseCmdOperands(lineInfo *line, int *IC, int *DC) {
    ...
    if (foundComma) {
        printError(line->lineNum, "a comma after the last parameter is invalid.");
        line->isError = TRUE;
        return;
    }
    ...
    while (!feof(file)) {
        if (readLine(file, lineStr, MAX_LINE_LENGTH + 2)) {
            ...
            if (*IC + *DC >= MAX_DATA_NUM) {
                ...
                printf("[Info] Memory is full. Stopping to read the file.\n");
                return ++errorsFound;
            }
            ++*linesFound;
        } else if (!feof(file)) {
